...
@IBDesignable
 class StarRatingView: UIView {
  ...
  @IBInspectable
   var emptyColor: UIColor = UIColor.lightGray {
    didSet {
      self.emptyStar = makeStarImage(pt: self.point, spacing: self.spacing, color: self.emptyColor, image: self.emptyImage)
      setNeedsDisplay()
    }
  ...
  private var fillStar: UIImage?
   ...
   func configure(_ attribute: StarRatingAttribute, current: CGFloat = 0, min: CGFloat = 0, max: CGFloat = 0) {
  ...
  private func makeStarImage(pt size: CGFloat, spacing: CGFloat, color: UIColor, image: UIImage?) -> UIImage? {
   ...
   override func draw(_ rect: CGRect) {
    ...
    private func updateLocation(_ location: CGPoint, _ shouldCallValueChanged: Bool = true) {
    switch self.type {
    case .rate:
